...
const Compiler = ({ roomId, userName: propUserName }) => {
  ...
  // Whiteboard state
  const [showFullscreenWhiteboard, setShowFullscreenWhiteboard] =
    useState(false);
      ...
      try {
        ...
        if (monacoRef.current && activeFileId && files[activeFileId]) {
          ...
          if (currentContent !== files[activeFileId]?.content) {
            ...
            const BACKEND_URL =
              ...
              "https://compiler-design.onrender.com";
            await fetch(
              `${BACKEND_URL}/api/sessions/${roomIdRef.current}/files`,
              {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
        ...
        if (monacoRef.current) {
          ...
          const contentToRestore =
            lastSavedContent && lastSavedContent !== content
              ? lastSavedContent
              : content;
  ...
  const pasteCode = async () => {
    ...
    const selection = monacoRef.current.getSelection();
    monacoRef.current.executeEdits("paste", [
  ...
  const shareLink = async () => {
    ...
    try {
      ...
      }/compiler?code=${encodeURIComponent(encodedContent)}`;
  ...
  const buildIceConfig = () => {
    ...
    if (inlineTurn.url && inlineTurn.username && inlineTurn.credential) {
      baseServers.push({
        urls: inlineTurn.url,
        username: inlineTurn.username,
        credential: inlineTurn.credential,
      });
    }
                    ...
                    if (roomUsers.some((user) => user.id === userId)) {
                      ...
                      const hasScreenShare =
                        ...
                        (localStream &&
                          localStream
                            .getVideoTracks()
                            .some((track) => track.label.includes("screen")));

                      const screenTrack = hasScreenShare
                        ? window.pendingScreenTrack
                        : null;

                      createPeerConnection(userId, true, screenTrack).catch(
                        (e) =>
                          console.error(
  ...
  const cleanupPeerConnection = (userId) => {
    if (peerConnections.current[userId]) {
      ...
      if (
        ...
        localStream.current &&
        window._autoStopVideoWhenAlone
      ) {
        ...
        if (currentContent !== files[activeFileId]?.lastSavedContent) {
          ...
          if (token) {
            const BACKEND_URL =
              import.meta.env.VITE_BACKEND_URL ||
              "https://compiler-design.onrender.com";
            const blob = new Blob([JSON.stringify(fileToSave)], {
  ...
  useEffect(() => {
    ...
    socketRef.current.on("connect", () => {
      ...
      if (!isFirstConnection) {
        ...
        if (localStream.current && roomsWithActiveCalls.current.has(roomId)) {
          ...
          setTimeout(() => {
            if (
              window._lastReadyEmit &&
              Date.now() - window._lastReadyEmit < 3000
            ) {
    ...
    socketRef.current.on("fileOpened", ({ fileId, name }) => {
      setFiles((prev) => ({
        ...prev,
        [fileId]: prev[fileId] || { name, content: "" },
      }));
    });
    ...
    socketRef.current.on("fileContentSnapshot", ({ fileId, name, content }) => {
      setFiles((prev) => ({
        ...prev,
        [fileId]: { name: name || fileId, content: content || "" },
      }));
    ...
    socketRef.current.on(
      "receiveCodeState",
      ({ code: receivedCode, language: receivedLanguage }) => {
    ...
    socketRef.current.on(
      "userReadyForCall",
      async ({ userId, userName: callerName }) => {
        ...
        if (
          ...
          now - lastInviteAtRef.current < 5000
        )
          return;
        lastInviteFromRef.current = userId;
              ...
              setTimeout(async () => {
                try {
                  if (
                    ...
                    const newOffer = await peerConnections.current[
                      from
                    ].createOffer({
                      offerToReceiveAudio: true,
                      offerToReceiveVideo: true,
  ...
  React.useEffect(() => {
    const setupEditor = () => {
      if (editorCreatedRef.current || !editorRef.current || !window.monaco)
        return;
      ...
      const monacoLang =
        monacoLanguageMappings[languageRef.current] || languageRef.current;
  ...
  useEffect(() => {
    if (socketRef.current && roomId) {
      ...
      socketRef.current.emit("openFile", {
        roomId,
        fileId: defaultId,
        name: defaultName,
      });
  ...
  const handleUploadFile = async (event) => {
    try {
      ...
      if (socketRef.current && roomIdRef.current) {
        ...
        // Set as active file
        socketRef.current.emit("setActiveFile", {
          roomId: roomIdRef.current,
          fileId,
        });
      }
        ...
        fileKeys.forEach((key) => {
          try {
            ...
            if (fileData.lastModified) {
              const age =
                Date.now() - new Date(fileData.lastModified).getTime();
              if (age > maxAge) {
    ...
    const restoreState = () => {
      try {
        ...
        const lastActiveFileId =
          localStorage.getItem("last_active_file") ||
          sessionStorage.getItem("last_active_file");
                  ...
                  setTimeout(() => {
                    ...
                    const countMessage =
                      newCount > currentCount
                        ? ` You now have ${newCount} saved file${
                            newCount > 1 ? "s" : ""
                          }.`
                        : "";
    ...
    <div className="flex h-screen bg-white dark:bg-black text-gray-900 dark:text-white overflow-hidden relative z-10">
      ...
      <div className="flex h-full">
        ...
        <motion.div
          ...
          } md:translate-x-0 fixed md:static z-30 w-16 bg-white/50 dark:bg-black/50 backdrop-blur-sm flex flex-col items-center py-4 border-r border-gray-200 dark:border-gray-800 transition-transform duration-300 h-full`}
        >
          {/* Users Tab */}
          {roomId && (
            <button
              ...
              className={`p-2.5 md:p-3 ${
                activeTab === "users"
                  ...
                  : "hover:bg-orange-100 dark:hover:bg-orange-900/20 text-gray-700 dark:text-gray-300 hover:text-orange-600 dark:hover:text-orange-400"
              } rounded-lg mb-3 md:mb-4 transition-all duration-200 hover:scale-105 relative`}
              title="Collaborators"
            >
              <svg
          ...
          <button
            ...
            className={`p-2.5 md:p-3 ${
              activeTab === "chat"
                ? "bg-orange-100 dark:bg-orange-900/40 text-orange-600 dark:text-orange-400"
                : "hover:bg-orange-100 dark:hover:bg-orange-900/20 text-gray-700 dark:text-gray-300 hover:text-orange-600 dark:hover:text-orange-400"
            } rounded-lg mb-3 md:mb-4 transition-all duration-200 hover:scale-105`}
            title="Chat"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
          ...
          <button
            ...
            className="p-2.5 md:p-3 hover:bg-orange-100 dark:hover:bg-orange-900/20 text-gray-700 dark:text-gray-300 hover:text-orange-600 dark:hover:text-orange-400 rounded-lg mb-3 md:mb-4 transition-all duration-200 hover:scale-105 relative"
                    ...
                    {roomUsers.length > 0 ? (
                      <div className="space-y-2">
                        {roomUsers.map((roomUser) => {
                          const isCurrentUser =
                            roomUser.id === socketRef.current?.id;
                          return (
                            <motion.div
                              key={roomUser.id}
                                ...
                                <div className="flex items-center gap-1">
                                  {typingUsers.has(roomUser.id) ? (
                                    <>
                                      ...
                                      <p className="text-xs text-orange-600 dark:text-orange-400 ml-1">
                                        typing...
                                      </p>
                                    </>
                                  ) : (
        ...
        <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
          ...
          <motion.div
            ...
            className={`flex-1 flex flex-col ${
              showOutput ? "hidden lg:flex" : "flex"
            }`}
          >
          ...
          <motion.div
            ...
            className={`w-full lg:w-1/3 bg-white/50 dark:bg-black/50 backdrop-blur-sm border-t lg:border-t-0 lg:border-l border-gray-200 dark:border-gray-800 flex flex-col ${
              showOutput ? "flex" : "hidden lg:flex"
            }`}
          >
            {/* Output Section */}
                ...
                <div className="space-y-4 sm:space-y-6">
                  ...
                  <div className="p-3 sm:p-4 bg-orange-50 dark:bg-orange-900/10 rounded-xl border border-orange-200 dark:border-orange-800">
                    <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-300">
                      ...
                      Theme preference is saved automatically and applies across
              ...
              <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl max-w-md w-full p-6 border border-gray-200 dark:border-gray-800">
                <div className="mb-4">
                  ...
                  <p className="text-gray-600 dark:text-gray-400">
                    Your changes will be lost. Would you like to save your code
                    to GitHub before exiting?
                  </p>
                </div>
