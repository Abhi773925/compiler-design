    ...
    <div>
      ...
      <div className="flex justify-end mb-4">
        <button
          ...
          className="text-sm bg-orange-600 hover:bg-orange-700 text-white py-1 px-3 rounded-md flex items-center transition-colors"
        >
            ...
            >
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-gray-600 dark:text-gray-400 truncate max-w-xs">{repo.description || 'No description'}</p>
                </div>
                <div className="flex items-center text-xs">
                  <span className={`inline-block w-2 h-2 rounded-full ${repo.private ? 'bg-red-500' : 'bg-green-500'}`}></span>
                  <span className="ml-1 text-gray-600 dark:text-gray-400">{repo.private ? 'Private' : 'Public'}</span>
                </div>
              </div>
            </motion.div>
          ))}
    ...
    <form onSubmit={handleCreateRepo}>
      ...
      <div className="flex justify-between mt-6">
        <button
          ...
          className="text-gray-600 dark:text-gray-400 hover:text-orange-600 dark:hover:text-orange-400 py-2 px-4 transition-colors"
        >
          Back
        </button>
        <button
          type="submit"
          disabled={loading}
          className="bg-orange-600 hover:bg-orange-700 text-white font-medium py-2 px-4 rounded-md disabled:opacity-50 transition-colors"
        >
  ...
  const renderSuccess = () => (
    <div className="flex flex-col items-center justify-center py-8">
      <div className="bg-green-100 dark:bg-green-900/30 rounded-full p-3 mb-4">
        <svg xmlns="http://www.w3.org/2000/svg" className="h-10 w-10 text-green-600 dark:text-green-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
        </svg>
      </div>
      <p className="text-lg font-medium text-green-600 dark:text-green-500 mb-1">Success!</p>
      <p className="text-center text-gray-700 dark:text-gray-300 mb-6">{success}</p>
      ...
      >
        ...
        {!isAuthenticated ? (
          <div className="text-center py-6">
            <p className="mb-6 text-gray-600 dark:text-gray-400">You need to connect with GitHub first</p>
            <a
              href="/github-callback?redirect=back"
              className="bg-orange-600 hover:bg-orange-700 text-white dark:bg-gray-800 dark:hover:bg-gray-700 dark:text-white font-medium py-2 px-4 rounded-md border border-orange-600 dark:border-gray-700 flex items-center mx-auto justify-center transition-all duration-200"
            >
